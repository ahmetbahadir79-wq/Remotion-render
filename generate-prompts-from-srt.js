...
const defaultSceneCount = Math.floor(Math.random() * (100 - 60 + 1)) + 60;
const sceneCount = parseInt(args['scenes'] || defaultSceneCount.toString());
...
for (let i = 0; i < sceneCount; i++) {
    ...
    const needsAccent = isChapterStart && characters.length > 0;
    if (needsAccent) {
        const characterPart = characters.length > 0 ? characters[0] : keywords.slice(0, 2).join(' and ');
        const accentPrompt = 'cinematic character portrait of ' + characterPart + ', isolated subject, transparent background, cutout style, sharp edges, bold contrast, no background, no environment, solid single color background, PNG alpha, high detail face and clothing, no accessories, no watermark';
        accentPrompts.push({ index: i, prompt: accentPrompt });
    }
...
if (accentPrompts.length > 0) {
    const accentFile = outputFile.replace('.txt', '-accent.txt');
    fs.writeFileSync(accentFile, accentPrompts.map(a => a.prompt).join('\n'), 'utf-8');
    console.log(`\n📝 ${accentPrompts.length} accent/cutout prompts saved to: ${accentFile}`);
}
